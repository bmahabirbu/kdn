...
func TestClaw_Name(t *testing.T) {
	...
	agent := NewClaw()
	if got := agent.Name(); got != "claw" {
		t.Errorf("Name() = %q, want %q", got, "claw")
	}
}
...
func TestClaw_SkipOnboarding_NoExistingSettings(t *testing.T) {
	...
	clawJSON, exists := result[ClawConfigPath]
	if !exists {
	...
	if auth["mode"] != "none" {
		t.Errorf("gateway.auth.mode = %v, want %q", auth["mode"], "none")
	}
...
func TestClaw_SkipOnboarding_PreservesExistingFields(t *testing.T) {
	...
	var config map[string]interface{}
	if err := json.Unmarshal(result[ClawConfigPath], &config); err != nil {
		t.Fatalf("Failed to parse result JSON: %v", err)
	}
	...
	if auth["otherOption"] != "preserved" {
		t.Errorf("gateway.auth.otherOption = %v, want %q", auth["otherOption"], "preserved")
	}
...
func TestClaw_SkipOnboarding_InvalidJSON(t *testing.T) {
	...
	settings := map[string][]byte{
		ClawConfigPath: []byte("invalid json {{{"),
	}
...
func TestClaw_SetMCPServers_NilMCP(t *testing.T) {
	...
	agent := NewClaw()
	settings := map[string][]byte{
		ClawConfigPath: []byte(`{"gateway":{"auth":false}}`),
	}
	...
	if string(result[ClawConfigPath]) != `{"gateway":{"auth":false}}` {
		t.Errorf("SetMCPServers() with nil MCP modified settings unexpectedly: %s", result[ClawConfigPath])
	}
}
...
func TestClaw_SetMCPServers_CommandBased(t *testing.T) {
	...
	if fsServer["transport"] != "stdio" {
		t.Errorf("transport = %v, want %q", fsServer["transport"], "stdio")
	}
...
func TestClaw_SetMCPServers_PreservesExistingFields(t *testing.T) {
	...
	result, err := agent.SetMCPServers(map[string][]byte{ClawConfigPath: existingJSON}, mcp)
	if err != nil {
		t.Fatalf("SetMCPServers() error = %v", err)
	}
...
func TestClaw_SetMCPServers_InvalidJSON(t *testing.T) {
	...
	_, err := agent.SetMCPServers(map[string][]byte{ClawConfigPath: []byte("invalid json {{{")}, mcp)
	if err == nil {
		t.Error("Expected error for invalid JSON, got nil")
	}
}
